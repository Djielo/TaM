...
Les workflows **génèrent** ce JSON puis **publient tout le site** sur la branche **`gh-pages`**
(plus de `git commit` automatique sur `master` pour les données).
...
- **`pages-sync-on-push.yml`** — à chaque **push** sur `master` : republie le site sur **gh-pages** en
  réinjectant le **dernier** `simulation_data.json` déjà en ligne (tes changements HTML/JS partent tout de suite).
...
3. Onglet **Actions** → lancer une première fois **Simulation — données Open Data (mensuel complet)** (*Run workflow* sur `master`) pour remplir **gh-pages** si la branche n’existait pas encore.
4. Attendre que le run soit **vert** ; ouvrir l’URL Pages (`…github.io/…/simulateur_sae.html`) et vérifier le simulateur.

Tant que **Pages** pointe vers **`gh-pages`**, les MàJ quotidiennes / mensuelles restent **automatiques** sans toucher à **`master`**.
